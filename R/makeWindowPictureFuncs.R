...
cleavageWindows <- function(dirO = paste0("pathOut/"), 
  ...
  transAndPosToBam <- function(
    ...
    readDepthBam <- function(posT,
      ...
      bamChr <- match(chr1,names(seqinfoBam))
      ...
      if(individual == T){
        ...
        if(aliFilesPatternCtrl != ""){
          ...
          # p4 <- tableGrob(printPage)
          # 
          # p3 <- do.call(grid.arrange, c(p2, ncol=2,
          #                         layout_matrix=list(cbind(1:(lenP/2),(lenP/2+1): lenP)
          #                         )))
          # egg::ggarrange
          # c(grid.arrange(p4),p3)
          # 
          lay1 <- rbind(c(1,1),
                        cbind(2:((lenP/2)+1),
                              (lenP/2+1): lenP+1))
          ...
          #grid.arrange(tableGrob(gridTab))
          gridTab2 <-list(tableGrob(gridTab,theme = mytheme))
          # gridTab <- list(tableGrob(printPage[,1:ceiling(ncolP/3)],theme = mytheme))
          # gridTab2 <- list(tableGrob(printPage[,(ceiling(ncolP/3)+1):(ceiling(ncolP/3*2))],theme = mytheme))
          # gridTab3 <- list(tableGrob(printPage[,(ceiling(ncolP/3*2)+1):ncolP],theme = mytheme))
          print(do.call(grid.arrange, c(c(gridTab2,p2),
                                        layout_matrix=list(lay1)
          )))
          
          # print(do.call(grid.arrange, c(p2, ncol=2,
          #                               layout_matrix=list(cbind(1:(lenP/2),(lenP/2+1): lenP)), top = paste(printPage,collapse = " "))
          #               ))
        }else{
          library(gridExtra)
          print("No Ctrl found")
    ...
    if(endsWith(aliFilesPath,suffix = "bamTranscript/")){
      ...
      chromPos1 <- gffTrans$V9[match(ChromPosT[,2],gffTrans$V10)]
      strand3 <- "+"
      
      
    }else {
      stop("Wrong filepath assigned")
    }
  ...
  for(x in(1:nrow(run1))){
    ...
    transAndPosToBam(PotInPotvsNoInf4 = run1[x,],#[3,]
