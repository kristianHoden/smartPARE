...
#' @export
#' @examples
#' kerasCreateDataset_2d <- function(homePath = "keras/3categories/", pixels = 28)

kerasCreateDataset_2d <- function(
  homePath = "keras/3categories/",
  pixels = 28){
  ...
  for (i in 1:length(testFiles)){testPics[[i]] <- readImage(files = testFiles[[i]])}
  #resize pictures
  for (i in 1:length(testFiles)){testPics[[i]] <-  EBImage::resize(testPics[[i]],pixels,pixels)}
  testPics <- combine(testPics)
...
kerasCreateModelCNN_2d <- function(unitPower = 3,
  ...
  model %>%
    layer_conv_2d(filters = 32, kernel_size = c(3,3),
                  ...
                  kernel_regularizer=regularizer_l1(1e-4))%>% #batch_size, c(3,3)
    layer_spatial_dropout_2d(dropout) %>%
  ...
  for(x in (1:unitPower)){
    ...
    model %>%
      layer_dense(units = units1,
                  activation = activation,
                  kernel_regularizer=regularizer_l2(1e-4)) %>% #l2 dense data, l1 sparse
      layer_dropout(dropout)
  }
  model %>%  
  ...
  for(examinePath1 in(allDirs)){
    ...
    while (picNrMax <= length(examineCleavageFilesOrig)) {
      ...
      for (i in 1:length(examineCleavageFiles)){
        ...
        #y <- 309
        examinePics[[i]] <- readImage(files = examineCleavageFiles[[i]])}
      print("resizing")
      for (i in 1:length(examineCleavageFiles)){
        percentageOfProcess(y = i,totalY = length(examineCleavageFiles),perc = 10)
      ...
      examinePics <- combine(examinePics)
      ...
      predValid_ex <- model %>% predict_classes(ex_valid)
...
runCLR <- function(optimizer2 = optimizer_sgd(lr=lr_max, decay=0),
                   unitPower2,
                   trainLabels2 = trainLabels,
                   epochs2 = 5,
                   batch_size2,
  ...
  rib <- function(code){
    ga2 <- mcparallelDo(code,targetValue = "ga")
    return(ga2)
  }
  LogMetrics <- R6::R6Class("LogMetrics",
                            inherit = keras::KerasCallback,
  ...
  earlyS <- EarlyStopping(patience = patience2)  
  model <- kerasCreateModelCNN_2d(optimizer = optimizer2,
  ...
  rib(
  model %>% save_model_hdf5(paste0(pathOut,as.character(count1),"_va_", valAcc,
                                   "_vl_", valLoss,
  ...
  library(ggplot2)
  meltedAcc
  p1 <- ggplot2::ggplot(meltedAcc, aes(x= xrange, y = value,group=variable,
                                   colour=variable)) +
    geom_line()+
    geom_point()+
    theme_bw()+
    scale_x_continuous( name="Epoch", limits=c(0, nrow(his)+1),expand = c(0, 0),breaks=seq(0,nrow(his),50) )+
    scale_y_continuous( name="Accuracy",expand = c(0, 0),limits=c(0,1))
  
  p1
  p2 <- ggplot2::ggplot(meltedLoss, aes(x= xrange,
                                        y = value,
                                        group=variable,
                                   colour=variable))+
    geom_line()+
    ...
    theme_bw()+
    scale_x_continuous( name="Epoch", limits=c(0, nrow(his)+1),expand = c(0, 0), breaks=seq(0,nrow(his),50))+
    scale_y_continuous( name="Loss",expand = c(0, 0))
  p2
  p3 <- cowplot::plot_grid(p1, p2, labels = c('A', 'B'))
  pdf(paste0(pathOut,"pdf/",as.character(count1),".pdf"))
  print(p3)
  dev.off()
...
#' saveDirLevelNr = 3 )
kerasListTrue <- function(pathToTrue = "keras/3categories/", saveDirLevelNr = 3 ){
  ###########################Explanation#####################
